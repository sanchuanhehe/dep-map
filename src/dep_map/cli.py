...
def load_or_scan(aports_path: Optional[str], use_cache: bool = True) -> DependencyGraph:
    """加载或扫描仓库"""
    ...
    console.print(f"[green]✓[/green] Scanned {result.successful} packages in {result.scan_time:.2f}s")
...
def scan(aports_path: str, repos: tuple, output: Optional[str]):
    """扫描 aports 仓库并构建依赖图谱"""
    console.print(Panel(f"[bold]Scanning aports repository[/bold]\n{aports_path}", border_style="blue"))
    ...
    with Progress(
        ...
        def progress_callback(current, total, name):
            progress.update(task, completed=current, total=total, description=f"[cyan]{name}[/cyan]")
    ...
    table.add_row("Most Dependencies", f"{stats['max_dependencies_package']} ({stats['max_dependencies_count']})")
...
@click.option("--type", "-t", "dep_type", type=click.Choice(["all", "runtime", "build"]), default="all")
@click.option("--tree", is_flag=True, help="以树形结构显示")
def deps(package: str, aports: Optional[str], recursive: bool, depth: int, dep_type: str, tree: bool):
    """查询软件包的依赖关系"""
    ...
    if tree:
        ...
        for dep in deps_list:
            ...
            if dep_pkg:
                table.add_row(
                    dep,
                    f"{dep_pkg.version}-r{dep_pkg.release}",
                    dep_pkg.repo
                )
            else:
...
def rdeps(package: str, aports: Optional[str], recursive: bool, depth: int):
    """查询软件包的反向依赖（哪些包依赖此包）"""
    graph = load_or_scan(aports)
...
def stats(aports: Optional[str], as_json: bool):
    """显示依赖图统计信息"""
    graph = load_or_scan(aports)
...
@click.option("--aports", "-a", type=click.Path(exists=True), help="aports 仓库路径")
def path(source: str, target: str, aports: Optional[str]):
    """查找两个包之间的依赖路径"""
    graph = load_or_scan(aports)
...
def serve(aports: Optional[str], port: int, host: str):
    """启动 Web 界面"""
    graph = load_or_scan(aports)
    ...
    try:
        ...
        console.print(f"[red]Error:[/red] Flask is required for web interface")
...
@click.argument("package")
@click.option("--aports", "-a", type=click.Path(exists=True), help="aports 仓库路径")
def info(package: str, aports: Optional[str]):
    """显示软件包详细信息"""
    graph = load_or_scan(aports)
